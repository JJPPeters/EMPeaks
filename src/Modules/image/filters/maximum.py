...
class MaximumFilterModule(MenuEntryModule):
    def __init__(self):
        super().__init__()
    ...
    def run(self):
        ...
        filter_settings = ProcessSettingsFrame(
            ...
            function=self.doMaximum,
            name="Maximum",
            inputs=[size, mode, cval, origin],
            show_preview=True, preserve_image=True)
        self.add_widget_to_image_window(filter_settings, 0, 2)

    def doMaximum(self, params):
        vals = params
        f = lambda i: ndimage.maximum_filter(i, size=vals[0], mode=vals[1], cval=vals[2], origin=vals[3])
