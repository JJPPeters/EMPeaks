...
class PrewittFilterModule(MenuEntryModule):
    def __init__(self):
        super().__init__()
    ...
    def run(self):
        ...
        filter_settings = ProcessSettingsFrame(
            ...
            function=self.doPrewitt,
            name="Prewitt",
            inputs=[axis, mode, cval],
            show_preview=True, preserve_image=True)
        self.add_widget_to_image_window(filter_settings, 0, 2)

    def doPrewitt(self, params):
        vals = params
        f = lambda i: ndimage.prewitt(i, axis=vals[0], mode=vals[1], cval=vals[2])
