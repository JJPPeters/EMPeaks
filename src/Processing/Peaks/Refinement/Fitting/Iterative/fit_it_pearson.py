...
from Processing.Utilities import pearson_vii_2d_fitting
...
def do_fitting_it_pearson(data, points, fit_r, contribute_r, iterations, lim=0, frac_change=0, do_init_fit=True, fit_method='leastsq'):
    image = normalise_copy(data)

    culled_points, initial_params = estimate_pearson_parameters(image, points, fit_r)
...
def estimate_pearson_parameter(image_region, r):
    ...
    param_dict['w_x'] = (sigma, 0.00001, np.inf)
...
def estimate_pearson_parameters(full_image, points, r):
    param_list = []
    ...
    for i, point in enumerate(points):
        ...
        param_list.append(estimate_pearson_parameter(image_region, r))
        good_points[i] = True
