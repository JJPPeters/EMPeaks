...
def do_fitting_it_lorentz(data, points, fit_r, contribute_r, iterations, lim=0, frac_change=0, do_init_fit=True, fit_method='leastsq'):
    image = normalise_copy(data)

    culled_points, initial_params = estimate_lorentz_parameters(image, points, fit_r)
...
def estimate_lorentz_parameter(image_region, r):
...
def estimate_lorentz_parameters(full_image, points, r):
    ...
    for i, point in enumerate(points):
        ...
        param_list.append(estimate_lorentz_parameter(image_region, r))
        good_points[i] = True
